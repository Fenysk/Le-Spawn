...
const hasInitialPurchasePrice = ref(false);
const setInitialPurchasePrice = () => {
    hasInitialPurchasePrice.value = true;
}

const hasNegotiatedPrice = ref(false);
const setNegotiatedPrice = () => {
    hasNegotiatedPrice.value = true;
    newGameData.value.negotiatedPurchasePrice = newGameData.value.initialPurchasePrice;
}

const hasEstimatedPrice = ref(false);
const setEstimatedPrice = () => {
    hasEstimatedPrice.value = true;
    newGameData.value.estimatedPrice = newGameData.value.initialPurchasePrice;
}
...
<template>
    <div v-show="!platformsLoading" class="lg:px-0">
        ...
        <form @submit.prevent="nextStep" v-show="step === 1" class="mt-4 flex flex-col gap-8">
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoFrontBox" :src="newGameData.photoFrontBox"
                                        alt="Photo de la boîte (face avant)" class="h-full w-full cursor-pointer object-cover"
                                    ...
                                    <div class="relative aspect-square overflow-hidden" v-else>
                                        <input type="file" id="photoFrontBox"
                                            class="absolute left-0 top-0 z-50 block h-full w-full cursor-pointer object-cover opacity-0"
                                            accept="image/jpeg, image/png, image/gif, image/webp"
                                            @change="handleFileChange('photoFrontBox', $event)"
                                            :disabled="imagesFiles.photoFrontBox.loading" />
                                        <div class="absolute left-0 top-0 z-40 flex h-full w-full cursor-pointer flex-col items-center justify-center bg-secondary"
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoBackBox" :src="newGameData.photoBackBox"
                                        alt="Photo de la boîte (face arrière)" class="h-full w-full cursor-pointer object-cover"
                                    ...
                                    <div class="relative aspect-square overflow-hidden" v-else>
                                        ...
                                        <div class="absolute left-0 top-0 z-40 flex h-full w-full cursor-pointer flex-col items-center justify-center bg-secondary text-xs"
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoSideBox" :src="newGameData.photoSideBox"
                                        alt="Photo de la boîte (côté)" class="h-full w-full cursor-pointer object-cover"
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoInsideBox" :src="newGameData.photoInsideBox"
                                        alt="Photo de la boîte (intérieur)" class="h-full w-full cursor-pointer object-cover"
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoFrontGame" :src="newGameData.photoFrontGame"
                                        alt="Photo du jeu (face avant)" class="h-full w-full cursor-pointer object-cover"
                            ...
                            <Card class="aspect-square overflow-hidden">
                                <CardContent class="h-full w-full p-0">
                                    <img v-if="newGameData.photoBackGame" :src="newGameData.photoBackGame"
                                        alt="Photo du jeu (face arrière)" class="h-full w-full cursor-pointer object-cover"
            ...
            <div class="flex flex-col px-4">
                ...
                <Button type="submit" :disabled="uploadLoading"
                    v-else-if="hasBoxImages && !hasGameImages && newGameData.stateBox !== 'NEW'">
                    <Loader2 class="mr-2 h-4 w-4 animate-spin" v-if="uploadLoading" />
        ...
        <form @submit.prevent="handleRegisterGame" v-show="step === 2"
            class="flex max-h-[70vh] flex-col gap-4 overflow-auto px-4">

            <pre class="h-48 w-[350px] overflow-auto">{{ newGameData }}</pre>
            ...
            <div class="flex flex-col gap-2">
                <Label for="edition" class="font-semibold">Édition</Label>
            ...
            <div class="flex flex-col gap-2">
                <Label for="region" class="font-semibold">Région</Label>
            ...
            <div>
                <Button v-if="!hasInitialPurchasePrice" variant="secondary" type="button"
                    @click="setInitialPurchasePrice" class="mt-2 w-full font-semibold">
                    Définir un prix d'achat initial
                </Button>

                <div class="flex flex-col gap-2" v-if="hasInitialPurchasePrice">
                    <Label for="initialPurchasePrice" class="font-semibold">Prix d'achat initial (en €)</Label>
                    <input type="number" v-model="newGameData.initialPurchasePrice"
                        class="rounded border p-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary"
                        required />
                </div>
            </div>

            <div v-if="hasInitialPurchasePrice">
                <Button v-if="!hasNegotiatedPrice" variant="secondary" type="button" @click="setNegotiatedPrice"
                    class="mt-2 w-full font-semibold">
                    J'ai négocié le prix
                </Button>

                <div class="mt-2 flex flex-col gap-2" v-if="hasNegotiatedPrice">
                    <Label for="negotiatedPurchasePrice" class="font-semibold">Prix d'achat négocié (en €)</Label>
                    <input type="number" v-model="newGameData.negotiatedPurchasePrice"
                        class="rounded border p-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary"
                        required />
                </div>
            </div>
            ...
            <div v-if="hasInitialPurchasePrice">
                <Button v-if="!hasEstimatedPrice" variant="secondary" type="button" @click="setEstimatedPrice"
                    class="mt-2 w-full font-semibold">
                    Définir un prix estimé
                </Button>

                <div class="mt-2 flex flex-col gap-2" v-if="hasEstimatedPrice">
                    <Label for="estimatedPrice" class="font-semibold">Prix estimé (en €)</Label>
                    <input type="number" v-model="newGameData.estimatedPrice"
                        class="rounded border p-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary"
                        required />
                </div>
            </div>
